...
var vulCommentPattern = regexp.MustCompile(`^\s*([/]+|/\*|#)+\s*vul:`)
...
func parseIgnoresFromLine(input string) []terraform.Ignore {
	...
	input = vulCommentPattern.ReplaceAllString(input, "vul:")
	...
	for i, bit := range bits {
		...
		if strings.HasPrefix(bit, "tfsec:") || strings.HasPrefix(bit, "vul:") {
...
func parseIgnoreFromComment(input string) (*terraform.Ignore, error) {
	...
	if !strings.HasPrefix(input, "tfsec:") && !strings.HasPrefix(input, "vul:") {
